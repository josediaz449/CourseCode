...
(define-for-syntax test-functions
  (list 'countdown
        'insertR
        'insertR-fr
        'remv-1st
...
(define-for-syntax (defined-function? x)
  (memv x test-functions))

(define-syntax (test-suite stx)
  (syntax-parse stx #:datum-literals (test-equal-if-defined)
    [(_ id
        (test-equal-if-defined f
                               [program expect])) #:when (defined-function? (syntax->datum #'f))
                                                  #'(begin (println (~a "testing " id))
                                                           (check-equal? program expect))]
    [(_ id
        (test-equal-if-defined f
                               [program expect]
                               [p2 e2] ...)) #:when (defined-function? (syntax->datum #'f))
                                             #'(begin
                                                 (println (~a "testing " id))
                                                 (check-equal? program expect)
                                                 (check-tests [p2 e2] ...))]))

(define-syntax (check-tests stx)
  (syntax-parse stx
    [(_ [program expect]) #'(check-equal? program expect)]
    [(_ [program expect]
        [p2 e2] ...) #'(begin (check-equal? program expect)
                              (check-tests [p2 e2] ...))]))
...
(define-syntax (check-answers* stx)
  (syntax-parse stx
    ...
    [(_ testsuite
        testsuite2 ...) #'(begin testsuite
                                 (check-answers* testsuite2 ...))]))


(define (check-answers)
  (check-answers*
